...
def _global_avg_pool2d(inputs, data_format='NHWC', scope=None, outputs_collections=None):
  with tf.compat.v1.variable_scope(scope, 'xx', [inputs]) as sc:
...
def _conv_block(inputs, num_filters, data_format='NHWC', scope=None, outputs_collections=None):
  with tf.compat.v1.variable_scope(scope, 'conv_blockx', [inputs]) as sc:
    net = inputs
...
def _dense_block(inputs, num_layers, num_filters, growth_rate,
  ...
  with tf.compat.v1.variable_scope(scope, 'dense_blockx', [inputs]) as sc:
    net = inputs
...
def _transition_block(inputs, num_filters, compression=1.0,
  ...
  with tf.compat.v1.variable_scope(scope, 'transition_blockx', [inputs]) as sc:
    net = inputs
...
def densenet(inputs,
  ...
  with tf.compat.v1.variable_scope(scope, 'densenetxxx', [inputs, num_classes],
                         reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      # final blocks
      with tf.compat.v1.variable_scope('final_block', [inputs]):
        net = slim.batch_norm(net)
        net = tf.nn.relu(net)
